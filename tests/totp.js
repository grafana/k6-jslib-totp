...
export default async function () {
    ...
    // Test at time=59 seconds, expected=94287082 (8 digits)
    const totp8 = new TOTP(secret, 8)
    const now = Math.floor(Date.now() / 1000)
    const bias59 = now - 59
    const code59 = await totp8.gen(30, bias59)
    check(code59, {
        'RFC6238 t=59s: 94287082': (c) => c === '94287082',
    })

    // Test at time=1111111109, expected=07081804 (8 digits)
    const bias1111111109 = now - 1111111109
    const code1111111109 = await totp8.gen(30, bias1111111109)
    check(code1111111109, {
        'RFC6238 t=1111111109: 07081804': (c) => c === '07081804',
    })

    // Test at time=1234567890, expected=89005924 (8 digits)
    const bias1234567890 = now - 1234567890
    const code1234567890 = await totp8.gen(30, bias1234567890)
    check(code1234567890, {
        'RFC6238 t=1234567890: 89005924': (c) => c === '89005924',
    })

    // Test at time=2000000000, expected=69279037 (8 digits)
    const bias2000000000 = now - 2000000000
    const code2000000000 = await totp8.gen(30, bias2000000000)
